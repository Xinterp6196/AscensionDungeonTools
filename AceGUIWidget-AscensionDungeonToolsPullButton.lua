local Type, Version = "AscensionDungeonToolsPullButton", 1
...
local methods = {
    ...
    ["Initialize"] = function(self)
        ...
        function self.callbacks.OnClickNormal(_, mouseButton)
            if not MouseIsOver(AscensionDungeonTools.main_frame.sidePanel.pullButtonsScrollFrame.frame) then return end
            ...
            elseif(IsShiftKeyDown()) then
                ...
                if(mouseButton == "RightButton") then
                    AscensionDungeonTools:SetMapSublevel(self.index)
                    AscensionDungeonTools:SetSelectionToPull(self.index)
                    L_EasyMenu(self.menu,AscensionDungeonTools.main_frame.sidePanel.optionsDropDown, "cursor", 0 , -15, "MENU")
                else
                    --normal click
                    AscensionDungeonTools:SetMapSublevel(self.index)
                    AscensionDungeonTools:SetSelectionToPull(self.index)
        ...
        function self.callbacks.OnEnter()
            ...
            AscensionDungeonTools.pullTooltip:SetPoint("TOPRIGHT",self.frame,"TOPLEFT",0,4)
            AscensionDungeonTools.pullTooltip:SetPoint("BOTTOMRIGHT",self.frame,"TOPLEFT",-250,-(4+AscensionDungeonTools.pullTooltip.myHeight))
            local tooltipBottom = AscensionDungeonTools.pullTooltip:GetBottom()
            local mainFrameBottom = AscensionDungeonTools.main_frame:GetBottom()
            if tooltipBottom<mainFrameBottom then
                AscensionDungeonTools.pullTooltip:SetPoint("TOPRIGHT",self.frame,"BOTTOMLEFT",0,(4+AscensionDungeonTools.pullTooltip.myHeight))
                AscensionDungeonTools.pullTooltip:SetPoint("BOTTOMRIGHT",self.frame,"BOTTOMLEFT",-250,-4)
            end
            AscensionDungeonTools:ActivatePullTooltip(self.index)
        end

        function self.callbacks.OnLeave()
            --model
            AscensionDungeonTools.pullTooltip.Model:Hide()
            --topString
        ...
        if self.index ~= 1 then
            tinsert(self.menu, {
                text = "Move up",
                notCheckable = 1,
                func = function() AscensionDungeonTools:MovePullUp(self.index) end
            })
        end
        if self.index<self.maxPulls then
            tinsert(self.menu, {
                text = "Move down",
                notCheckable = 1,
                func = function() AscensionDungeonTools:MovePullDown(self.index) end
            })
        end
        tinsert(self.menu, {
            text = "Clear",
            notCheckable = 1,
            func = function() AscensionDungeonTools:ClearPull(self.index) end
        })
        tinsert(self.menu, {
            text = " ",
            notClickable = 1,
        ...
        if self.maxPulls > 1 then
            tinsert(self.menu, {
                text = "Delete",
                notCheckable = 1,
                func = function() AscensionDungeonTools:DeletePull(self.index) end
            })
        end
        tinsert(self.menu, {
            text = " ",
        ...
        tinsert(self.menu, {
            text = "Close",
            notCheckable = 1,
            func = AscensionDungeonTools.main_frame.sidePanel.optionsDropDown:Hide()
        })
...
local function Constructor()
    local name = "AscensionDungeonToolsPullButton"..AceGUI:GetNextWidgetNum(Type);
    ...
    for i=1,maxPortraitCount do
        ...
        enemyPortraits[i].overlay:SetTexture("Interface\\Addons\\AscensionDungeonTools\\Textures\\Circle_White")
